...
int lcd_set_mode(unsigned int *option)
{
	...
	if( option ){
		...
		// Mode set in transmitter
		if(!(get_lcd_setting() || transmitter_enable())){
		...
		govrh_set_dvo_sync_polar(vo->govr,(lcd_panel_cap & LCD_CAP_HSYNC_HI)? 0:1,
			(lcd_panel_cap & LCD_CAP_VSYNC_HI)? 0:1);
